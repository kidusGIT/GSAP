...
let _win,
  ...
  _setterCSSStyle = (target, property, value) =>
    (target.style[property] = value),
  _setterCSSProp = (target, property, value) =>
    target.style.setProperty(property, value),
  _setterTransform = (target, property, value) =>
    (target._gsap[property] = value),
  _setterScale = (target, property, value) =>
    (target._gsap.scaleX = target._gsap.scaleY = value),
  ...
  _saveStyle = function (property, isNotCSS) {
    ...
    if (property in _transformProps && style) {
      ...
      if (property !== "transform") {
        ...
        ~property.indexOf(",")
          ? property.split(",").forEach((a) => (this.tfm[a] = _get(target, a)))
        ...
        return _propertyAliases.transform
          .split(",")
          .forEach((p) => _saveStyle.call(this, p, isNotCSS));
      }
  ...
  _getStyleSaver = (target, properties) => {
    ...
    properties &&
      target.style &&
      target.nodeType &&
      properties.split(",").forEach((p) => saver.save(p)); // make sure it's a DOM node too.
    return saver;
  },
  _supports3D,
  ...
  _getReparentedCloneBBox = (target) => {
  ...
  _isSVG = (e) =>
    !!(e.getCTM && (!e.parentNode || e.ownerSVGElement) && _getBBox(e)), //reports if the element is an SVG on which getBBox() actually works
  ...
  _specialProps = {
    clearProps(plugin, target, property, endValue, tween) {
      if (tween.data !== "isFromStart") {
        let pt = (plugin._pt = new PropTween(
          plugin._pt,
          target,
          property,
          0,
  ...
  _addRotationalPropTween = function (
    plugin,
    target,
    property,
    startNum,
...
_forEachName("padding,margin,Width,Radius", (name, index) => {
  let t = "Top",
    ...
    props = (index < 3 ? [t, r, b, l] : [t + l, t + r, b + r, b + l]).map(
      (side) => (index < 2 ? name + side : "border" + side + name),
    );
  _specialProps[index > 1 ? "border" + name : name] = function (
    ...
    vars = {};
    props.forEach(
      (prop, i) => (vars[prop] = a[i] = a[i] || a[((i - 1) / 2) | 0]),
    );
...
export {
  CSSPlugin as default,
  _getBBox,
  _createElement,
  _checkPropPrefix as checkPrefix,
};
