...
let gsap,
  ...
  _keepClamp = (value, clamp) =>
    clamp && (!_isString(value) || value.substr(0, 6) !== "clamp(")
      ? "clamp(" + value + ")"
      : value,
  ...
  _getViewportDimension = (dimensionProperty) =>
  ...
  _getSizeFunc = (scroller, isViewport, { d, d2, a }) =>
    (a = _getProxyProp(scroller, "getBoundingClientRect"))
      ? () => a()[d]
      : () =>
          (isViewport ? _getViewportDimension(d2) : scroller["client" + d2]) ||
          0,
  ...
  _endAnimation = (animation, reversed, pause) =>
    animation &&
  ...
  _getLabelAtDirection = (timeline) => (value, st) =>
    _snapDirectional(_getLabelRatioArray(timeline))(value, st.direction),
  _multiListener = (func, element, types, callback) =>
    types.split(",").forEach((type) => func(element, type, callback)),
  _addListener = (element, type, func, nonPassive, capture) =>
  ...
  _clearScrollMemory = (scrollRestoration, force) => {
    ...
    (force || !_refreshingAll) &&
      _scrollers.forEach(
        (obj) => _isFunction(obj) && obj.cacheID++ && (obj.rec = 0)
      );
  ...
  _refreshAll = (force, skipRevert) => {
    ...
    _scrollers.forEach(
      (obj) => _isFunction(obj) && ++obj.cacheID && (obj.rec = obj())
      ...
      if (_isFunction(obj)) {
        obj.smooth &&
          requestAnimationFrame(
            () => (obj.target.style.scrollBehavior = "smooth")
          );
        obj.rec && obj(obj.rec);
      }
    });
    ...
    _updateAll(2);
    _triggers.forEach(
      (t) => _isFunction(t.vars.onRefresh) && t.vars.onRefresh(t)
    );
  ...
  _getTweenCreator = (scroller, direction) => {
    let getScroll = _getScrollFunc(scroller, direction),
      ...
      getTween = (scrollTo, vars, initialValue, change1, change2) => {
        ...
        let checkForInterruption = _interruptionTracker(
          getScroll,
          initialValue,
          () => {
            tween.kill();
...
export class ScrollTrigger {
      ...
      if (invalidateOnRefresh && animation) {
        ...
        animation.getChildren &&
          animation
            ...
            .forEach((t) => t.vars.immediateRender && t.render(0, true, true)); // any from() or fromTo() tweens inside a timeline should render immediately (well, unless they have immediateRender: false)
      }
      ...
      let i = _triggers.indexOf(self),
        a =
          self.direction > 0
            ...
            : _triggers.slice(i + 1);
      return (
        _isString(name) ? a.filter((t) => t.vars.preventOverlaps === name) : a
      ).filter((t) => (self.direction > 0 ? t.end <= start : t.start >= end));
    };
  ...
  static clearMatchMedia(query) {
    _triggers.forEach(
      (t) => t._ctx && t._ctx.query === query && t._ctx.kill(true, true)
    );
  }
  ...
  static killAll(allowListeners) {
    _triggers
      .slice(0)
      .forEach((t) => t.vars.id !== "ScrollSmoother" && t.kill());
...
ScrollTrigger.update = (force) =>
  ++_scrollers.cache && _updateAll(force === true ? 2 : 0);
...
ScrollTrigger.getScrollFunc = (element, horizontal) =>
  _getScrollFunc(_getTarget(element), horizontal ? _horizontal : _vertical);
ScrollTrigger.getById = (id) => _ids[id];
ScrollTrigger.getAll = () =>
  _triggers.filter((t) => t.vars.id !== "ScrollSmoother"); // it's common for people to ScrollTrigger.getAll(t => t.kill()) on page routes, for example, and we don't want it to ruin smooth scrolling by killing the main ScrollSmoother one.
...
let _clampScrollAndGetDurationMultiplier = (scrollFunc, current, end, max) => {
  ...
  _inputObserver = (target, type, inputs, nested) =>
    Observer.create({
      target: target,
      capture: true,
      debounce: false,
      ...
      onDisable: () =>
        _removeListener(_doc, Observer.eventTypes[0], _captureInputs, true),
    }),
  ...
  _getScrollNormalizer = (vars) => {
    ...
    tween = gsap.to(self, {
      ...
      scrollY: "+=0.1",
      modifiers: {
        scrollY: _interruptionTracker(scrollFuncY, scrollFuncY(), () =>
          tween.pause()
        ),
      },
      onUpdate: _updateAll,
