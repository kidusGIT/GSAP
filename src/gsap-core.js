...
let _config = {
  ...
  _isTypedArray =
    (typeof ArrayBuffer === "function" && ArrayBuffer.isView) || function () {}, // note: IE10 has ArrayBuffer, but NOT ArrayBuffer.isView().
  ...
  _getProperty = (target, property, v) =>
    (v = target[property]) && _isFunction(v)
      ...
      : (_isUndefined(v) &&
        ...
        v,
  _forEachName = (names, func) =>
    (names = names.split(",")).forEach(func) || names, //split a comma-delimited list of names into an array, then run a forEach() function and return the split array (this is just a way to consolidate/shorten some code).
  ...
  _isRevertWorthy = (animation) =>
    !!(animation._initted || animation._startAt || animation.add),
  _lazySafeRender = (animation, time, suppressEvents, force) => {
    _lazyTweens.length && !_reverting && _lazyRender();
    animation.render(
      time,
      suppressEvents,
      force || !!(_reverting && time < 0 && _isRevertWorthy(animation)),
    );
  ...
  _rewindStartAt = (tween, totalTime, suppressEvents, force) =>
    tween._startAt &&
    (_reverting
    ...
    let labels = animation.labels,
      ...
      clippedDuration =
        animation.duration() >= _bigNum
          ...
          : animation._dur, //in case there's a child that infinitely repeats, users almost never intend for the insertion point of a new child to be based on a SUPER long value like that so we clip it and assume the most recently-added child's endTime should be used instead.
      i,
      offset,
      isPercent;
  ...
  getUnit = (value, v) =>
    !_isString(value) || !(v = _unitExp.exec(value)) ? "" : v[1], // note: protect against padded numbers as strings, like "100.100". That shouldn't return "00" as the unit. If it's numeric, return no unit.
  clamp = (min, max, value) =>
    _conditionalReturn(value, (v) => _clamp(min, max, v)),
  _slice = [].slice,
  ...
  toArray = (value, scope, leaveStrings) =>
    _context && !scope && _context.selector
  ...
  shuffle = (a) => a.sort(() => 0.5 - Math.random()), // alternative that's a bit faster and more reliably diverse but bigger:   for (let j, v, i = a.length; i; j = (Math.random() * i) | 0, v = a[--i], a[i] = a[j], a[j] = v); return a;
  ...
  unitize = (func, unit) => (value) =>
    func(parseFloat(value)) + (unit || getUnit(value)),
  normalize = (min, max, value) => mapRange(min, max, 0, 1, value),
  _wrapArray = (a, wrapper, value) =>
    _conditionalReturn(value, (index) => a[~~wrapper(index)]),
  wrap = function (min, max, value) {
    ...
    return _isArray(min)
      ...
      : _conditionalReturn(
          value,
          (value) => ((range + ((value - min) % range)) % range) + min,
        );
  },
  ...
  _configElastic = (type, amplitude, period) => {
    let p1 = amplitude >= 1 ? amplitude : 1, //note: if amplitude is < 1, we simply adjust the period for a more natural feel. Otherwise the math doesn't work right and the curve starts at 1.
      ...
      easeOut = (p) =>
        p === 1 ? 1 : p1 * 2 ** (-10 * p) * _sin((p - p3) * p2) + 1,
      ease =
        type === "out"
          ? easeOut
          : type === "in"
...
export class Animation {
  ...
  timeScale(value, suppressEvents) {
    ...
    this.totalTime(
      _clamp(-Math.abs(this._delay), this.totalDuration(), tTime),
...
export class Timeline extends Animation {
  constructor(vars = {}, position) {
    ...
    _globalTimeline &&
      _addToTimeline(vars.parent || _globalTimeline, this, position);
            ...
            child.render(
              child._ts > 0
                ...
                : (child._dirty ? child.totalDuration() : child._tDur) +
                    (adjustedTime - child._start) * child._ts,
              suppressEvents,
              force || (_reverting && _isRevertWorthy(child)),
...
_forEachName(
  ...
  (name) => (_staggerPropsToSkip[name] = 1),
);
...
export class Tween extends Animation {
  constructor(targets, vars, position, skipInherit) {
    ...
    if (
      ...
      (!duration &&
        !keyframes &&
        this._start === _roundPrecise(parent._time) &&
  ...
  render(totalTime, suppressEvents, force) {
    ...
    if (!dur) {
      ...
      if (!prevTime && tTime && !suppressEvents && !prevIteration) {
        ...
        console.log("this._repeat ", this._repeat);
        _callback(this, "onStart");
        if (this._tTime !== tTime) {
          // in case the onStart triggered a render at a different spot, eject. Like if someone did animation.pause(0.5) or something inside the onStart.
          return this;
  ...
  kill(targets, vars = "all") {
    if (!targets && (!vars || vars === "all")) {
      ...
      this.parent
        ? _interrupt(this)
        : this.scrollTrigger && this.scrollTrigger.kill(!!_reverting);
      return this;
    }
    if (this.timeline) {
...
let _setterPlain = (target, property, value) => (target[property] = value),
  ...
  _setterFuncWithParam = (target, property, value, data) =>
    target[property](data.fp, value),
  _setterAttribute = (target, property, value) =>
...
class Context {
  ...
  add(name, func, scope) {
    ...
    return name === _isFunction
      ? f(self, (func) => self.add(null, func))
      : name
        ? (self[name] = f)
        : f;
  }
  ...
  kill(revert, matchMedia) {
    if (revert) {
      ...
      while (i--) {
        ...
        if (t.data === "isFlip") {
          t.revert();
          t.getChildren(true, true, false).forEach((tween) =>
      ...
      tweens
        ...
        .sort((a, b) => b.g - a.g || -Infinity)
        .forEach((o) => o.t.revert(revert)); // note: all of the _startAt tweens should be reverted in reverse order that they were created, and they'll all have the same globalTime (-1) so the " || -1" in the sort keeps the order properly.
      i = this.data.length;
      while (i--) {
...
const _gsap = {
  ...
  quickTo(target, property, vars) {
    let tween = gsap.to(
        target,
        _setDefaults(
          { [property]: "+=0.1", paused: true, stagger: 0 },
          vars || {},
        ),
      ),
